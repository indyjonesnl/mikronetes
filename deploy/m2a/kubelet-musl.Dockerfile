...
# Musl-static standalone rusternetes kubelet for M2b worker microVM overlays.
#
# Build context is the parent of rusternetes:
#   docker build -f mikronetes/deploy/m2a/kubelet-musl.Dockerfile \
#     -t mikronetes-kubelet:m2b /home/jones/PhpstormProjects
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --profile release-fast --features sqlite -p rusternetes-kubelet && \
    mkdir -p /out && cp target/release-fast/kubelet /out/kubelet && \
    sccache --show-stats

FROM alpine:3.21
WORKDIR /app
COPY --from=builder /out/kubelet /app/kubelet
